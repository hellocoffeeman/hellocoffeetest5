...
//    CGFloat oriY = self.cropFrame.origin.y + (self.cropFrame.size.height - oriHeight) / 2;
    ...
    CGFloat oriHeight = self.originalImage.size.height * (oriWidth / self.originalImage.size.width);
    CGFloat oriX = self.view.frame.origin.x + (self.view.frame.size.width - oriWidth) / 2;
    CGFloat oriY = self.view.frame.origin.y + (self.view.frame.size.height - oriHeight) / 2;
...
//        }];
//    }
    self.showImgView.bounds = CGRectMake(0, 0, self.showImgView.frame.size.width*pinchGestureRecognizer.scale, self.showImgView.frame.size.height*pinchGestureRecognizer.scale);
    [pinchGestureRecognizer setScale:1];//不能清0
}
