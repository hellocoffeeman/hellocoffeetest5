...
//    NSLog(@" PageScrollView .contentSize = %@",NSStringFromCGSize(self.contentSize));
    ...
    if (showPageControl == YES) {
        self.pageControlLocation = pageControlLocation;
...
-(void)showPageControlWithPageControlLocation:(PageControlLocation)pageControlLocation numberOfPage:(int)numberOfPage
{
    ...
    switch (pageControlLocation) {
        case PageControlLocationRight:
            self.pageControl.center = CGPointMake(self.size.width-40+self.contentOffset.x, self.size.height-12);//s_width/kTopPictureRatio - 12);
            break;
        case PageControlLocationCenter:
            self.pageControl.center = CGPointMake(s_width/2+self.contentOffset.x, self.size.height-12);//s_width/kTopPictureRatio - 12);
            break;
    }
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    switch (self.pageControlLocation) {
        case PageControlLocationRight:
            self.pageControl.center = CGPointMake(s_width-40+self.contentOffset.x,self.size.height-12);// s_width/kTopPictureRatio - 12);
            break;
        case PageControlLocationCenter:
            self.pageControl.center = CGPointMake(s_width/2+self.contentOffset.x, self.size.height-12);//s_width/kTopPictureRatio - 12);
            NSLog(@"SRC----s_width/2+self.contentOffset.x=%f,self.contentOffset.x==%f",s_width/2+self.contentOffset.x,self.contentOffset.x);
            break;
    }
}
...
-(void)setContentOffset:(CGPoint)contentOffset
{
    ...
    int currentPage = contentOffset.x/self.size.width;
    self.pageControl.currentPage = currentPage-1;
}
